...
// Type of theme key
export Space = SpaceThemeKeys | (string & {})
export Color = ColorThemeKeys | (string & {})
export FontSize = FontSizeThemeKeys | (string & {})
export LetterSpacing = LetterSpacingThemeKeys | (string & {})
export LineHeight = LineHeightThemeKeys | (string & {})
...
/** 
 * Convert to css property and value from Responsive type
 * @param propKey: CSS property
 * @param prop: Responsive type
 * @param theme: AppTheme
 * @return CSS property and value
 * **/
  ...
  if (isResponsivePropType(prop)) {
    ...
    for (const responsiveKey in prop) {
      if (responsiveKey === 'base') {
        // default style
        result.push(`${propKey}: ${toThemeValueNeeded(propKey, prop[responsiveKey], theme,)};`,)
      } else if (
        responsiveKey === 'sm' ||
        responsiveKey === 'md' ||
        responsiveKey === 'lg' ||
        ...
        // Style in media queries
        const breakpoint = BREAKPOINTS[responsiveKey]
        const style = `${propKey}: ${toThemeValueNeeded(propKey, prop[responsiveKey], theme,)};`
        result.push('@media screen and(min - width: ${breakpoint}) {${style} } ')
      }
    }
    return result.join('\n')
  }
  return `${propKey}: ${toThemeValueNeeded(propKey, prop, theme)};`
}
...
/**
 * Converts to CSS property values specified for the theme
 * @param propKey: CSS property
 * @param value: CSS value
 * @param theme: AppTheme
 * @return CSS property values specified
**/
function toThemeValueNeeded<T>(propKey: string, value: T, theme?: AppTheme) {
  ...
  } else if (theme && theme.lineHeight && LINE_HEIGHT_KEYS.has(propKey) && isLineHeightThemeKeys(value, theme)) {
